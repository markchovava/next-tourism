...
export default function GuideList() {
    ...
    async function getData() {
        try{
        const result = await axiosClientAPI.get(`guide`, config)
        .then((response) => {
    ...
    async function searchData() {
        ...
        try{
        const result = await axiosClientAPI.get(`guide?search=${search}`, config)
        .then((response) => {
            setData(response.data.data)
            setPrevURL(response.data.links.prev)
            setNextURL(response.data.links.next)
    ...
    async function deleteData(id) {
        try{
        const result = await axiosClientAPI.delete(`guide/${id}`, config)
        .then((response) => {
    ...
    <div>
      ...
      <section className='w-[100%]'>
        <div className='w-[90%] mx-auto text-lg py-2 flex items-center justify-start font-bold font-white bg-slate-200 '>
          ...
          <div className='w-[25%] border-r border-white px-3 py-2'>SLUG</div>
        ...
        {data.length > 0 ? 
          data.map((item, i) => (
            <div key={i} className='w-[90%] text-lg border-x border-b border-slate-300 mx-auto flex items-center justify-start '>
              <div className='w-[30%] border-r border-blue-300 px-3 py-2 flex items-center justify-between gap-2'>
              ...
              <div className='w-[25%] border-r border-blue-300 px-3 py-2 flex items-center justify-start gap-2'>
                  {item?.user?.name ? item?.user?.name : item?.user?.email}
              </div>
              <div className='w-[20%] px-3 py-2 flex items-center justify-start gap-4 text-xl'>
                <Link href={`/admin/guide/${item.id}`}> <FaEye className='hover:text-blue-500 duration-150 hover:scale-110 transition-all ease-in'/> </Link>
                <Link href={`/admin/guide/edit/${item.id}`}> <MdEdit className='hover:text-green-500 duration-150 hover:scale-110 transition-all ease-in' /> </Link>  
                <button 
