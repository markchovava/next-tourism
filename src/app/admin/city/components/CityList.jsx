...
export default function CityList() {
    ...
    async function getData() {
        try{
        const result = await axiosClientAPI.get(`city`, config)
        .then((response) => {
    ...
    async function searchData() {
        ...
        try{
        const result = await axiosClientAPI.get(`city?search=${search}`, config)
        .then((response) => {
            setData(response.data.data)
            setPrevURL(response.data.links.prev)
            setNextURL(response.data.links.next)
    ...
    async function deleteData(id) {
        try{
        const result = await axiosClientAPI.delete(`city/${id}`, config)
        .then((response) => {
          ...
          data.map((item, i) => (
            <div key={i} className='w-[90%] border-x border-b border-slate-300 mx-auto flex items-center justify-start '>
              ...
              <div className='w-[15%] px-3 py-2 flex items-center justify-start gap-4'>
                <Link href={`/admin/city/${item.id}`}> <FaEye className='hover:text-blue-500 duration-150 hover:scale-110 transition-all ease-in'/> </Link>
                <Link href={`/admin/city/edit/${item.id}`}> <MdEdit className='hover:text-green-500 duration-150 hover:scale-110 transition-all ease-in' /> </Link>  
                <button 
