...
export default function ProvinceList() {
    ...
    async function getData() {
        try{
        const result = await axiosClientAPI.get(`province`, config)
        .then((response) => {
    ...
    async function searchData() {
        ...
        try{
        const result = await axiosClientAPI.get(`province?search=${search}`, config)
        .then((response) => {
            setData(response.data.data)
            setPrevURL(response.data.links.prev)
            setNextURL(response.data.links.next)
    ...
    async function deleteData(id) {
        try{
        const result = await axiosClientAPI.delete(`province/${id}`, config)
        .then((response) => {
    ...
    <div>
      ...
      <section className='w-[100%]'>
        <div className='w-[90%] mx-auto text-lg py-2 flex items-center justify-start font-bold font-white bg-slate-200 '>
          <div className='w-[60%] border-r border-white px-3 py-2'>Name</div>
        ...
        {data.length > 0 ? 
          data.map((item, i) => (
            <div key={i} className='w-[90%] border-x border-b border-slate-300 mx-auto flex items-center justify-start '>
              <div className='w-[60%] border-r border-blue-300 px-3 py-2 flex items-center justify-between gap-2'>
              ...
              <div className='w-[20%] px-3 py-2 flex items-center justify-start gap-4'>
                <Link href={`/admin/province/${item.id}`}> <FaEye className='hover:text-blue-500 duration-150 hover:scale-110 transition-all ease-in'/> </Link>
                <Link href={`/admin/province/edit/${item.id}`}> <MdEdit className='hover:text-green-500 duration-150 hover:scale-110 transition-all ease-in' /> </Link>  
                <button 
