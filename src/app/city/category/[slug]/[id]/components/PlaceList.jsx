...
import { FaAngleRight, FaArrowLeftLong, FaArrowRightLong, FaHeart, FaRegHeart, FaRegStar, FaStar } from "react-icons/fa6";
import { IoIosSearch } from "react-icons/io";

export default function PlaceList({places, city, category_slug, city_slug}) {
  ...
  async function getData() {
      try{
          const result = await axios.get(`${baseURL}city-category-places?category_slug=${category_slug}&city_slug=${city_slug}`)
          .then((response) => {
              setData(response.data.data)
              setPrevURL(response.data.links.prev)
              setNextURL(response.data.links.next)
  ...
  async function searchData() {
      ...
      try{
          const result = await axios.get(`${baseURL}city-category-places?category_slug=${category_slug}&city_slug=${city_slug}&search=${search}`)
          .then((response) => {
              setData(response.data.data)
              setPrevURL(response.data.links.prev)
              setNextURL(response.data.links.next)
