...
export default function GuidePlaceList({city, dbData, guide_slug, city_slug}) {
    console.log(dbData);
    ...
    async function getData() {
        try{
            const result = await axios.get(`${baseURL}place-city-guide?guide_slug=${guide_slug}&city_slug=${city_slug}`)
            .then((response) => {
                setData(response.data.data)
                setPrevURL(response.data.links.prev)
                setNextURL(response.data.links.next)
    ...
    async function searchData() {
        ...
        try{
            const result = await axios.get(`${baseURL}place-city-guide?guide_slug=${guide_slug}&city_slug=${city_slug}&search=${search}`)
            .then((response) => {
                setData(response.data.data)
                setPrevURL(response.data.links.prev)
                setNextURL(response.data.links.next)
    ...
    <div>
        ...
        <section className='w-[100%]'>
            <div className='mx-auto w-[90%] border-b border-slate-200'>
                <ul className='flex items-center justify-start gap-2 py-2'>
                    ...
                    <li><FaAngleRight /></li>
                    <li><Link href='/city'>Province</Link></li>                               
                    <li><FaAngleRight /></li>
                    <li><Link href={`/city/guide/${city_slug}/${guide_slug}`}>
        ...
        <section className='w-[100%]'>
            <div className='mx-auto w-[90%] flex items-center justify-center flex-col pt-[10rem] pb-[5rem]'>
                <h6>Find a place to visit in {city?.data?.name}.</h6>
