...
export default function CityPlaces({slug, city, places }) {
    ...
    async function getData() {
        try{
            const result = await axios.get(`${baseURL}city-places?slug=${slug}`)
            .then((response) => {
                setData(response.data.data)
                setPrevURL(response.data.links.prev)
                setNextURL(response.data.links.next)
    ...
    async function searchData() {
        ...
        try{
            const result = await axios.get(`${baseURL}city-places?slug=${slug}&search=${search}`)
            .then((response) => {
                setData(response.data.data)
                setPrevURL(response.data.links.prev)
                setNextURL(response.data.links.next)
        ...
        <section className='w-[100%]'>
            <div className='mx-auto w-[90%] border-b border-slate-200'>
                <ul className='flex items-center justify-start gap-2 px-3 py-2'>
                    ...
                    <li><FaAngleRight /></li>
                    <li><Link href={`/city/${slug}`}>{city?.data?.name}</Link></li>                               
                </ul>
            </div>
        </section>
        {/*  */}
