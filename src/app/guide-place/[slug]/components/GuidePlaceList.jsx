...
import { FaRegHeart, FaHeart, FaArrowRightLong, FaArrowLeftLong, FaAngleRight, FaStar, FaRegStar } from "react-icons/fa6";
import { IoIosSearch } from 'react-icons/io';



export default function GuidePlaceList({ dbData, slug, guideData }) {
    const [data, setData] = useState(dbData?.data);
    const [guide, setGuide] = useState(guideData?.data);
    ...
    async function getData() {
        try{
            const result = await axios.get(`${baseURL}place-guide-by-slug?slug=${slug}`)
            .then((response) => {
                setData(response.data.data)
                setPrevURL(response.data.links.prev)
                setNextURL(response.data.links.next)
    ...
    async function searchData() {
        ...
        try{
            const result = await axios.get(`${baseURL}place-guide-by-slug?slug=${slug}&search=${search}`)
            .then((response) => {
                setData(response.data.data)
                setPrevURL(response.data.links.prev)
                setNextURL(response.data.links.next)
            ...
            <div className='mx-auto w-[90%] flex items-center justify-center flex-col pt-[10rem] pb-[5rem]'>
                ...
                <div className="w-[80%] mx-auto border border-slate-300 rounded-full overflow-hidden flex items-center justify-start">
                    <input 
                        ...
                        placeholder="Search GuidePlaces by name..." />
