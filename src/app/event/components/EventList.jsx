...
export default function EventList({ eventsData }) {
    const [data, setData] = useState(eventsData.data);
    const [search, setSearch] = useState('');
    const [isSearch, setIsSearch] = useState(false);
    const [nextURL, setNextURL] = useState(eventsData.links?.next)
    const [prevURL, setPrevURL] = useState(eventsData.links?.prev)
    ...
    <div>
        ...
        <section className='w-[100%]'>
            <div className='mx-auto w-[90%] flex items-center justify-center flex-col pt-[10rem] pb-[5rem]'>
                <h6>Find your a event to visit</h6>
                    ...
                    {data.map((i, key) => (
                        <div key={key} className='relative group w-[100%] rounded-lg overflow-hidden aspect-[5/4] bg-slate-400 mb-3'>
                            ...
                            <div className='absolute bottom-0 left-0 w-[100%] h-[50%] text-white text-[2rem] font-bold flex items-end px-3 pb-4'>
                                <Link href={`/event/${i.id}`} className='link__two'>
                                    { i.name.length > 15 ? i.name.substring(0, 20) + '...' : i.name } 
                                </Link> 
                            </div>
                        </div>
                    ))}
